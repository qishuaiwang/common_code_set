...
enum data_repackage_index
{
    ...
    R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, R16, BA0, BA1, BA2, Rank
};
#else
struct MODULE_BIT_INFO ddr_module_32gb[DDR_MODULE_32GBIT_BIT_NUM] = {
    {"B0" ,  0,  0,  2,  0},
...
enum data_repackage_index
{
    B0 = 0, B1, B2, B3, C0, C1, C2, C3, C4, C5, BA0, BA1, BA2, R0,  
    R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, R16, Rank
};
#endif
...
int hif_addr_update (__uint32_t *addrmap)
{
    ...
    ddr_module_32gb[BA2].hif_position = (0x3f == GET_ADDRMAP_BANK_B2(addrmap[1])) ? 0x3f : GET_ADDRMAP_BANK_B2(addrmap[1]) + 5;
...
__uint64_t module_addr_get (struct FILE_INFO *p_file_info, __uint64_t index)
{
    ...
    module_addr = (p_file_info->rank_num << ddr_module_32gb[Rank].module_position) | index;
    return module_addr;
}
...
int create_mem_load_cmd (FILE *p_file, struct FILE_INFO *p_file_info)
{
    ...
    fprintf(p_file, hieraych_string, p_file_info->sys_num, p_file_info->rank_num, p_file_info->ch_name);
    fprintf(p_file, file_string, cur_dir, p_file_info->sys_num, p_file_info->rank_num, p_file_info->ch_name, p_file_info->index, p_file_info->start_offset);
}
...
/* or new file if file_creating = 0, file_creating = 1*/
        if (!p_file_info->increasing) {
            ...
            p_file_info->rank_num, p_file_info->ch_name,
            p_file_info->index);
